...
- `styled-components`는 기존 UI 임시 이식을 위해 남길 수 있으나, 최종적으로 CSS Modules 전환 완료 후 제거한다. Next.js 마이그레이션 앱에서는 제거 완료 상태다.
...
React Compiler는 최종 Next.js 앱에서 사용한다. 라우팅, Server/Client Component 경계, legacy styled-components 제거, 상태관리, DB/API 전환 문제와 원인을 분리하기 위해 마이그레이션 마지막 단계에서 별도 커밋으로 활성화한다.
...
- `next.config.ts`에 `reactCompiler: { compilationMode: "infer" }`를 추가한다.
...
## 2026-07-01 최종 정리 상태
- 공개 프론트, private-reader 인증, DB/Turso 전환, Agent 읽기 API, Admin CRUD, React Compiler 활성화를 완료했다.
- 앱 런타임 데이터 원본은 DB이며, `src/server/db/seed-data/*`는 초기 bootstrap/복구용 데이터로 유지한다.
- R2 콘텐츠는 저장 필드가 아니라 규칙 기반 경로(`novel/{id}.md`, `comics/{comicId}/`)로 조회한다.
- Next.js 앱에서는 `styled-components` 의존성을 제거했고, Radix UI + CSS Modules 조합을 기준으로 유지한다.
- 기존 Vite CSR 앱 삭제와 `nextjs-migration/` 루트 승격은 사용자가 직접 진행한다.

배포 전 확인 체크리스트:
- 배포 환경변수 `TURSO_*`, `R2_*`, `PRIVATE_READER_*`, `ADMIN_*`, `AGENT_API_TOKEN`을 설정한다.
- 배포 환경에서 `npm run build`를 실행해 React Compiler와 Next.js 빌드가 통과하는지 확인한다.
- 주요 페이지(`/`, `/novel/[id]`, `/comic/[id]`, `/heartbeat`, `/onthestage`, `/players`, `/private`, `/admin`)를 확인한다.
- R2 Markdown/이미지 로딩, private-reader 인증, admin CRUD, Agent API 인증을 확인한다.
...
- 커밋 메시지는 한국어로 간단히 작성하며, 제목과 본문을 포함해 3줄 이내로 제한한다.

예시 커밋:
```txt
Next.js 마이그레이션 가이드 추가
```
